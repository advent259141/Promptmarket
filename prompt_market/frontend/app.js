// 全局变量
const API_BASE_URL = 'http://www.jasongjz.top:8000/api/v1';
let currentPage = 1;
...
// 加载Prompt列表
async function loadPrompts(page = 1, searchTerm = '', sortBy = 'upload_time_desc') { // 默认 sortBy 也更改
    try {
        showLoading(true);
        
        // 构建URL参数
        const skip = (page - 1) * ITEMS_PER_PAGE;
        let url = `${API_BASE_URL}/prompts/?skip=${skip}&limit=${ITEMS_PER_PAGE}`;
        ...
        // 添加R18筛选参数
        if (currentR18Filter === "r18-only") {
            url += `&r18=1`;
        } else if (currentR18Filter === "non-r18") {
            url += `&r18=0`;
        }
        ...
        // 更新分页信息
        updatePagination(page, prompts.length < ITEMS_PER_PAGE);
    } catch (error) {
            ...
            authorHtml = `
                <div class="card-author">
                    <img src="${avatarUrl}" alt="${username}" class="author-avatar" 
                         ...
                         onerror="this.src='/app/default-avatar.jpg'">
    ...
    try {
        ...
        if (tryChatBtn) {
            tryChatBtn.onclick = function() {
                ...
                // 检查openChatWindow函数是否存在（在chat.js中定义）
                if (typeof openChatWindow === 'function') {
                    openChatWindow(prompt.id, prompt.title);
                } else {
